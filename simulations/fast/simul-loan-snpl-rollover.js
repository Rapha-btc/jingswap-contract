...
async function main() {
  ...
  const sessionId = await SimulationBuilder.new({ skipTracing: true })
    .useBlockHeight(blockPins["simul-loan-snpl-rollover"].block_height)
    .withSender(LENDER)
  ...
  console.log(`\nSession: ${sessionId}`);
  const _verify = await verifyAndReport(sessionId, "LOAN SNPL ROLLOVER", expectations["simul-loan-snpl-rollover"] || {});
