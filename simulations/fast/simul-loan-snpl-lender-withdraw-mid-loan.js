...
async function main() {
  ...
  const sessionId = await SimulationBuilder.new({ skipTracing: true })
    .useBlockHeight(blockPins["simul-loan-snpl-lender-withdraw-mid-loan"].block_height)
    .withSender(LENDER)
  ...
  console.log(`\nSession: ${sessionId}`);
  const _verify = await verifyAndReport(sessionId, "LOAN SNPL LENDER WITHDRAW MID LOAN", expectations["simul-loan-snpl-lender-withdraw-mid-loan"] || {});
