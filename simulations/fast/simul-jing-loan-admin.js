...
async function main() {
  ...
  const sessionId = await SimulationBuilder.new({ skipTracing: true })
    .useBlockHeight(blockPins["simul-jing-loan-admin"].block_height)
    .withSender(LENDER)
    .addContractDeploy({
  ...
  console.log(`\nSession: ${sessionId}`);
  const _verify = await verifyAndReport(sessionId, "JING LOAN ADMIN", expectations["simul-jing-loan-admin"] || {});
