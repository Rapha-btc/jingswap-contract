// simul-blind-premium-zero-same-depositor.js
// Stxer simulation: same address deposits on both STX and sBTC sides, then settle
//
// Run: npx tsx simulations/simul-blind-premium-zero-same-depositor.js
...
async function main() {
  ...
  console.log("=== BLIND PREMIUM - SAME DEPOSITOR BOTH SIDES ===\n");
  ...
  const sessionId = await SimulationBuilder.new()
    ...
    // Read both limits for same depositor
    .addEvalCode(CONTRACT_ID, `(get-stx-limit '${DEPOSITOR})`)
    .addEvalCode(CONTRACT_ID, `(get-sbtc-limit '${DEPOSITOR})`)
    ...
    .addEvalCode(CONTRACT_ID, `(get-stx-deposit u1 '${DEPOSITOR})`)
    .addEvalCode(CONTRACT_ID, `(get-sbtc-deposit u1 '${DEPOSITOR})`)
