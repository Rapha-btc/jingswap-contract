// simul-blind-premium-zero-priority-queue.js
// Stxer simulation: test priority queue bumping with MAX_DEPOSITORS=5
//
// Run: npx tsx simulations/simul-blind-premium-zero-priority-queue.js
...
async function main() {
  let source = fs.readFileSync("./contracts/blind-premium-zero-stxer.clar", "utf8");
  // MAX_DEPOSITORS is already u5 in blind-premium-stxer (same as blind-premium)

  console.log("=== BLIND PREMIUM - PRIORITY QUEUE BUMPING TEST (MAX_DEPOSITORS=5) ===\n");
  ...
  for (let i = 0; i < 4; i++) {
    sim = sim.withSender(ADDR[i]).addContractCall({
      ...
      function_args: [uintCV(STX_NORMAL), uintCV(LIMIT_STX)],
    });
  }
  sim = sim.withSender(ADDR[4]).addContractCall({
    contract_id: CONTRACT_ID,
    function_name: "deposit-stx",
    function_args: [uintCV(STX_SMALLEST), uintCV(LIMIT_STX)],
  });
  ...
  for (let i = 0; i < 4; i++) {
    sim = sim.withSender(ADDR[i]).addContractCall({
      ...
      function_args: [uintCV(SBTC_NORMAL), uintCV(LIMIT_SBTC)],
    });
  }
  sim = sim.withSender(ADDR[4]).addContractCall({
    contract_id: CONTRACT_ID,
    function_name: "deposit-sbtc",
    function_args: [uintCV(SBTC_SMALLEST), uintCV(LIMIT_SBTC)],
  });
  ...
  // ---- 6th STX: too small -> should FAIL ----
  sim = sim.withSender(ADDR[5]).addContractCall({
    contract_id: CONTRACT_ID,
    function_name: "deposit-stx",
    function_args: [uintCV(STX_BUMPER_FAIL), uintCV(LIMIT_STX)],
  });

  // ---- 6th STX: 3 STX -> should SUCCEED, bumps ADDR[4] ----
  sim = sim.withSender(ADDR[6]).addContractCall({
    contract_id: CONTRACT_ID,
    function_name: "deposit-stx",
    function_args: [uintCV(STX_BUMPER_OK), uintCV(LIMIT_STX)],
  });
  ...
  sim = sim.withSender(ADDR[5]).addContractCall({
    contract_id: CONTRACT_ID,
    function_name: "deposit-sbtc",
    function_args: [uintCV(SBTC_BUMPER_FAIL), uintCV(LIMIT_SBTC)],
  });
  ...
  sim = sim.withSender(ADDR[7]).addContractCall({
    contract_id: CONTRACT_ID,
    function_name: "deposit-sbtc",
    function_args: [uintCV(SBTC_BUMPER_OK), uintCV(LIMIT_SBTC)],
  });
  ...
  sim = sim
    ...
    .addEvalCode(CONTRACT_ID, `(get-stx-deposit u0 '${ADDR[4]})`) // bumped -> 0
    .addEvalCode(CONTRACT_ID, `(get-stx-deposit u0 '${ADDR[6]})`) // new -> 3 STX
    .addEvalCode(CONTRACT_ID, `(get-sbtc-deposit u0 '${ADDR[4]})`) // bumped -> 0
    .addEvalCode(CONTRACT_ID, `(get-sbtc-deposit u0 '${ADDR[7]})`) // new -> 3000 sats
    .addEvalCode(CONTRACT_ID, "(get-cycle-totals u0)");

  // Verify bumped depositor's limit was cleared
  sim = sim.addEvalCode(CONTRACT_ID, `(get-stx-limit '${ADDR[4]})`);
