// simul-v3-dust-sweep.js
// Stxer simulation: dust sweep verification for v3 (sBTC/USDCx instance).
// 3 depositors per side with amounts that maximize integer truncation dust.
//
// Run: npx tsx simulations/simul-v3-dust-sweep.js
...
async function main() {
  ...
  console.log("=== V3 DUST SWEEP - SBTC/USDCX STXER SIM ===\n");
  ...
  // Fund all addresses with STX (gas)
  ...
  for (const [addr, amount] of [
    [USDCX_D1, USDCX_D1_AMOUNT], [USDCX_D2, USDCX_D2_AMOUNT], [USDCX_D3, USDCX_D3_AMOUNT],
  ]) {
  ...
  // 3 sBTC depositors
  for (const [addr, amount] of [
    [SBTC_D1, SBTC_D1_AMOUNT], [SBTC_D2, SBTC_D2_AMOUNT], [SBTC_D3, SBTC_D3_AMOUNT],
  ]) {
    sim = sim.withSender(addr).addContractCall({
