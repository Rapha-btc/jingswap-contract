...
;; (.token-x-token-y-jing-v3, x=sBTC, y=USDCx) with a Bitflow
...
;;   (.token-x-token-y-jing-v3), into Bitflow's
...
(define-public (cancel-jing-usdcx)
  (begin
    ...
    (try! (contract-call? .jing-core log-cancel
      .token-x-token-y-jing-v3 USDCX_TOKEN))
    (ok true)))
